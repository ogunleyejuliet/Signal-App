    ...
    <Card borderVariant="subtle" hoverEffect className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-white border border-slate-200 text-rose-700">
            <Bot className="w-4 h-4" />
          </div>
          <span className="text-xs font-mono font-bold text-slate-600">{aiProvider}</span>
        </div>
        <AiRecommendationStatus status={status} rankText={rank ? `#${rank}` : undefined} size="sm" />
      </div>

      <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 font-mono text-xs text-rose-800">
        &quot;{query}&quot;
      </div>
      ...
      <div className="flex items-center justify-end">
        <button className="text-[11px] font-semibold text-rose-700 hover:text-rose-900 inline-flex items-center gap-1 cursor-pointer">
