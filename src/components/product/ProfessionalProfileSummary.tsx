    ...
    <Card borderVariant="wine" className="space-y-4">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-white">How Signal AI Understands You</h3>
        </div>
      ...
      <div className="flex items-start gap-4">
        <div className="relative shrink-0">
          <div className="w-14 h-14 rounded-2xl bg-rose-900/60 border border-rose-700/50 flex items-center justify-center text-white font-black text-xl shadow-lg shadow-rose-950">
            {profile.name.split(' ').map(n => n[0]).join('')}
          </div>
          <span className="absolute -bottom-1 -right-1 p-1 rounded-full bg-emerald-500 border-2 border-slate-950 text-slate-950">
            <ShieldCheck className="w-3 h-3" />
          </span>
        </div>

        <div className="space-y-1 min-w-0 flex-1">
          <h4 className="text-base font-extrabold text-white truncate">{profile.name}</h4>
          <p className="text-xs font-semibold text-rose-300 truncate">{profile.title}</p>
